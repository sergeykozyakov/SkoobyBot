...
class Sender extends BaseAction
{
    public function start() {
        if (!$this->getApi()) {
            $this->getLogger()->error('Cannot start sending messages until Telegram API is connected!');
        ...
        try {
            ...
            $this->getLogger()->error('Cannot execute cron VK API import: ' . $e->getMessage());
            throw new \Exception('[ERROR] Cannot execute cron VK API import: ' . $e->getMessage());
        }
    }
}
